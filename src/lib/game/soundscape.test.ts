...
describe('soundscape profile', () => {
  it('begins with only a quiet outdoor wind layer', () => {
    expect(soundscapeProfile(defaultQuest())).toEqual({ wind: 0.024, rail: 0, birds: 0, roomTone: 0 })
  })
  ...
  it('adds the restored world layers at the correct milestones', () => {
    ...
    expect(soundscapeProfile(quest)).toMatchObject({ rail: 0.012, birds: 0.045 })
    expect(soundscapeProfile(quest, true)).toMatchObject({ wind: 0.008, birds: 0, roomTone: 0.018 })
  })
})
